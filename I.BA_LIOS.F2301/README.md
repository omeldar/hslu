...
3.  Tests mit Operatoren:
    Sie können verschiedene Vergleichsoperatoren verwenden, um Bedingungen zu überprüfen. Zum Beispiel: - **`eq`**: gleich - **`ne`**: ungleich - **`lt`**: kleiner als - **`gt`**: größer als - **`le`**: kleiner oder gleich - **`ge`**: größer oder gleich
...
2.  Verwendung von regulären Ausdrücken:
    Mit regulären Ausdrücken können Sie komplexere Muster suchen. Hier sind einige Beispiele: - Suche nach einem Wort mit einer bestimmten Länge: **`grep -E '\b\w{5}\b' datei.txt`** - Suche nach Zeilen, die mit einem bestimmten Muster beginnen: **`grep '^muster' datei.txt`** - Suche nach Zeilen, die mit einem bestimmten Muster enden: **`grep 'muster$' datei.txt`** - Suche nach Zeilen, die ein bestimmtes Muster enthalten: **`grep 'muster' datei.txt`**
