...
\begin{itemize}
    ...
    \item $L \subseteq \Sigma_I^{*}$: Sprache der zulässigen Eingaben. Menge aller sinnvollen Eingaben/Instanzen. Ein $x \in L$ heisst Problemfall/Instanz. Wichtig: Man betrachtet hier bewusst das Optimierungsproblem und nimmt an, dass Eingaben ausserhalb von $L$ nicht vorkommen.
    ...
    \item $\text{goal} \in \{\text{Minimum, Maximum}\}$: Optimierungsziel. Legt fest, ob die kleinsten oder grössten Kosten gesucht werden (Minimierungs- vs. Maximierungsproblem).
\end{itemize}

Damit ist eine Lösung $\alpha \in M(x)$ optimal, wenn sie das Ziel erreicht:
$$
\text{cost}(\alpha, x) = \text{Opt}_U(x) = \text{goal} \{ \text{cost}(\beta, x) \mid \beta \in M(x) \}
$$
...
\begin{multicols}{2}
    ...
    $$ $$ 
    = \text{goal}\{\text{cost}(\beta, x) \mid \beta \in M(x)\}
    $$
    \vspace{0.5cm}
    $\phantom{a}$
    \columnbreak
...
1 - (1 - p)^k
$$
Das ist genau die Idee der Wahrscheinlichkeitsverstärkung durch Wiederholungen: Die Erfolgswahrscheinlichkeit steigt schnell, während die Laufzeit nur um den Faktor $k$ (linear) wächst.
...
\begin{multicols}{2}
    ...
    \textit{In der Abbildung gibt es eine Kante die keinen blauen Knoten berührt (ganz links, Abbildung 6.7)}\newline
    \textbf{Kosten}: Die Anzahl der Knoten in $U (|U|)$.\newline
    \textbf{Ziel}: Minimum
...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \textbf{Antwort}: Man führt den randomisierten Algorithmus mehrmals unabhängig auf derselben Instanz aus und behält am Ende die beste gefundene zulässige Lösung (bei Minimum: kleinster Wert, bei Maximum: grösster Wert).

        \item Was können wir über die Wahrscheinlichkeit für die optimale Lösung sagen?\newline
        \textbf{Antwort}: Wenn ein einzelner Lauf das Optimum mit Wahrscheinlichkeit mindestens $p$ findet, dann ist nach $t$ unabhängigdn Läufen die Erfolgswahrscheinlichkeit $1 - (1-p)^t$. Falls: $p \geq \frac{1}{|x|}$, dann liefern $|x|$ Wiederholungen schon konstante Erfolgswahrscheinlichkeit (mind. $1 - 1 / e$), und mit $|x| \log |x|$ geht sie sehr nahe zu 1 (100\%).

        \item Welche Eigenschaft soll eine zulässige Lösung haben?\newline
        \textbf{Antwort}: Sie muss alle Nebenbedingungen/Constraints der Instanz erfüllen, also wirklich eine erlaubte Lösung des Problemfalls sein.

        \item Wie wird ein Optimierungsproblem allgemein beschrieben?\newline
        \textbf{Antwort}: Ein Optimierungsproblem wird durch (1) Menge zulässiger Eingaben, (2) Menge zulässiger Lösungen pro Eingabe, (3) Kosten-/Bewertungsfunktion und (4) Zielrichtung (Minimum oder Maximum) beschrieben.
    \end{enumerate}
    \item Beispiele:
    \begin{enumerate}[label=(\roman*)]
        \item Kann ich das TSP-Problem erklären?\newline
        \textbf{Antwort}: Eingabe: gewichteter Graph. Z.B. Städte mit Distanzen. Zulässige Lösungen: Rundreisen, die jede Stadt genau einmal besuchen (Hamiltonkreis). Ziel: Gesamtlänge minimieren.

        \item Kann ich das MIN-VCP-Problem erklären?\newline
        \textbf{Antwort}: Eingabe: Graph. Gesucht ist eine möglichst kleine Knotenmenge $U \subseteq V$, sodass jede Kante in $E$ an mindestens einem Endpunkt in $U$ liegt. Ziel: $U$ minimieren.

        \item Kann ich das MAX-SAT-Problem erklären?\newline
        \textbf{Antwort}: Eingabe: Formel in KNF (UND von Klauseln). Lösung: Wahrheitsbelegung der Variablen. Ziel: Anzahl erfüllter Klauseln maximieren.

        \item Kann ich erklären, was IPL-Probleme sind?\newline
        \textbf{Antwort}: Man hat lineare Nebenbedingungen (Gleichungen/Ungleichungen) und eine lineare Zielfunktion. Gesucht ist eine optimale Lösung, bei der die Variablen ganzzahlig (z.B. in $\Z$ oder $\N$) sein müssen. Ziel: Zielfunktion minimieren oder maximieren.
